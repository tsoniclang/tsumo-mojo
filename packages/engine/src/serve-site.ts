...
import { ServeRequest, serveBuildRequest } from "./build.js";
...
const startWatchLoop = (req: ServeRequest, onRebuild: (outputDir: string) => void): void => {
  ...
  setInterval(() => {
    ...
    rebuilding = true;
    try {
      const result = buildSite(serveBuildRequest(req));
      onRebuild(result.outputDir);
      logLine(`[tsumo] rebuilt → ${result.outputDir}`);
    } catch (error) {
      const message = error instanceof TsumoError
        ? error.diagnostic.format()
        : "Unexpected non-Tsumo rebuild failure";
...
export const serveSite = (req: ServeRequest): void => {
  ...
  let outputDir = buildSite(serveBuildRequest(req)).outputDir;
