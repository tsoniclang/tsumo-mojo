...
import { testRegularExpression } from "./regular-expressions.js";
import {
  codePointAtText,
  indexOfText,
  nextCodePointIndex,
...
const parseInteger = (value: string, invalid: StructuredScalarErrorFactory): ParamValue | undefined => {
  const integerLike = testRegularExpression("^[+-]?[0-9_]+$", value);
  if (!integerLike) return undefined;
  if (!testRegularExpression("^[+-]?(?:0|[1-9](?:_?[0-9])*)$", value)) {
...
export const stripStructuredComment = (line: string, format: StructuredScalarFormat): string => {
  ...
  for (let index: int32 = 0; index < line.length; index = nextCodePointIndex(line, index)) {
    ...
    if (escaped) {
      escaped = false;
      previousWasWhitespace = testRegularExpression("\\s", current);
      continue;
    }
    if (quote === "\"" && current === "\\") {
      escaped = true;
