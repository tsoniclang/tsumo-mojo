...
import { testRegularExpression } from "../../utils/regular-expressions.js";
import {
  AnyArrayValue,
  BoolValue,
  DictValue,
...
const splitTomlKey = (
  ...
  for (let index: int32 = 0; index <= text.length; index++) {
    ...
    if (raw.startsWith("\"") || raw.startsWith("'")) {
      ...
      if (!testRegularExpression("^[A-Za-z0-9_-]+$", raw)) {
...
class TomlValueReader {
  ...
  parseBareScalar(): TemplateValue {
    ...
    while (this.index < this.text.length) {
      const character = this.peek();
      if (character === "," || character === "]" || character === "}" || testRegularExpression("\\s", character)) break;
      this.index++;
    }
    ...
    try {
      ...
      if (
        testRegularExpression("^\\d{4}-\\d{2}-\\d{2}(?:[Tt ][0-9:.+-]+[Zz]?)?$", raw) ||
        testRegularExpression("^\\d{2}:\\d{2}:\\d{2}(?:\\.\\d+)?$", raw)
      ) {
  ...
  skipWhitespace(): void {
    while (this.index < this.text.length && testRegularExpression("\\s", this.text[this.index]!)) this.index++;
  }
