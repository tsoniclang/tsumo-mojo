...
export class ResourcePipelineTests {
  ...
  image_resize_uses_the_exact_mojo_platform_boundary(): void {
    ...
    const resized = resizeImageResource(source, "2x3 png");
    Assert.NumberEqual(2, resized.width);
    Assert.NumberEqual(3, resized.height);
    Assert.StringEqual("pixel_2x3.png", resized.outputRelPath);
    const dimensions = parseImageDimensions(resized.bytes);
    Assert.True(dimensions !== undefined && dimensions.width === 2 && dimensions.height === 3);
  }
...
export const runResourcePipelineTests = (): void => {
  ...
  runTest("image resize uses the exact Mojo platform boundary", () => {
    tests.image_resize_uses_the_exact_mojo_platform_boundary();
  });
